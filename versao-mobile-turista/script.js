...
function toggleCheck(id){ const item = checks.find(c => c.id === id); if(item){ item.done = !item.done; renderChecklist(); } }
function deleteCheck(id){ checks = checks.filter(c => c.id !== id); renderChecklist(); }
...
function deleteLink(id){ links = links.filter(l => l.id !== id); renderLinks(); }
